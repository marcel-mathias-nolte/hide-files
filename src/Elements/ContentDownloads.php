 ...
 */
class ContentDownloads extends \Contao\ContentElement
{
	/**
	 * Files object
	...
	protected function compile()
	{
		...
		while ($objFiles->next())
		{
			...
			if (isset($files[$objFiles->path]) || !file_exists(\Contao\System::getContainer()->getParameter('kernel.project_dir') . '/' . $objFiles->path))
			{
				continue;
			}
			...
			if ($objFiles->type == 'file')
			{
				...
				if (!$arrMeta['title'])
				{
					$arrMeta['title'] = \Contao\StringUtil::specialchars($objFile->basename);
				}
				...
				$strHref .= (strpos($strHref, '?') !== false ? '&amp;' : '?') . 'file=' . \Contao\System::urlEncode($objFiles->path) . '&amp;cid=' . $this->id;
				...
				(
					...
					'title'     => \Contao\StringUtil::specialchars(sprintf($GLOBALS['TL_LANG']['MSC']['download'], $objFile->basename)),
					'link'      => $arrMeta['title'],
					'caption'   => $arrMeta['caption'],
					'href'      => $strHref,
					'filesize'  => $this->getReadableSize($objFile->filesize),
					'icon'      => \Contao\Image::getPath($objFile->icon),
					'mime'      => $objFile->mime,
					'meta'      => $arrMeta,
			...
			else
			{
				...
				while ($objSubfiles->next())
				{
					...
					$strHref .= (strpos($strHref, '?') !== false ? '&amp;' : '?') . 'file=' . \Contao\System::urlEncode($objSubfiles->path);
